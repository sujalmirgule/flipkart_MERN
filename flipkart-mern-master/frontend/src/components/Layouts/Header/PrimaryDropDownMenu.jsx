...
const PrimaryDropDownMenu = ({ setTogglePrimaryDropDown, user }) => {
    ...
    return (
        <div className="absolute w-60 -left-24 ml-2 top-9 bg-white shadow-2xl rounded flex-col text-sm">
            ...
            {navs.map((item, i) => {
                const { title, icon, redirect, disabled } = item;

                if (disabled) {
                    return (
                        <div className="pl-3 py-3.5 border-b flex gap-3 items-center text-gray-400 cursor-not-allowed select-none" key={i}>
                            <span className="text-gray-400">{icon}</span>
                            <span>{title}</span>
                            <span className="ml-auto mr-3 text-[10px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded border">Soon</span>
                        </div>
                    );
                }
