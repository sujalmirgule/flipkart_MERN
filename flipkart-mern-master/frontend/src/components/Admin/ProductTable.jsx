...
const ProductTable = () => {
    ...
    const { loading: productsLoading, products, error } = useSelector((state) => state.products);
    const { loading: deleteLoading, isDeleted, error: deleteError } = useSelector((state) => state.product);
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => {
                return (
                    <span>₹{params.row.price?.toLocaleString() || "0"}</span>
                );
            },
        },
        {
            ...
            renderCell: (params) => {
                return (
                    <span>₹{params.row.cprice?.toLocaleString() || "0"}</span>
                );
            },
        },
        {
            ...
            headerAlign: "left",
            renderCell: (params) => {
                return <Rating readOnly value={params.row.rating || 0} size="small" precision={0.5} />
            }
        },
        {
            field: "actions",
    ...
    safeProducts.forEach((item) => {
        ...
        rows.unshift({
            ...
            image: item.images?.[0]?.url || "https://rukminim1.flixcart.com/image/416/416/kapWA680/mobile/p/b/s/apple-iphone-11-mhdh3hn-a-original-imafvfeyycsbvzcw.jpeg",
            category: item.category || "General",
            stock: item.stock ?? 0,
            price: item.price || 0,
            cprice: item.cuttedPrice || 0,
    ...
    return (
        <>
            ...
            {(productsLoading || deleteLoading) && <BackdropLoader />}
