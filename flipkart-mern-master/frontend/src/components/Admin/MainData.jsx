        ...
        datasets: [
            {
                ...
                data: months.map((m, i) => safeOrders.filter((od) => od.createdAt && new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear() - 2).reduce((total, od) => total + (od.totalPrice || 0), 0)),
            },
            {
                label: `Sales in ${date.getFullYear() - 1}`,
                borderColor: 'orange',
                backgroundColor: 'orange',
                data: months.map((m, i) => safeOrders.filter((od) => od.createdAt && new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear() - 1).reduce((total, od) => total + (od.totalPrice || 0), 0)),
            },
            {
                label: `Sales in ${date.getFullYear()}`,
                borderColor: '#4ade80',
                backgroundColor: '#4ade80',
                data: months.map((m, i) => safeOrders.filter((od) => od.createdAt && new Date(od.createdAt).getMonth() === i && new Date(od.createdAt).getFullYear() === date.getFullYear()).reduce((total, od) => total + (od.totalPrice || 0), 0)),
            },
        ],
    };
        ...
        datasets: [
            {
                ...
                data: statuses.map((status) =>
                    safeOrders.filter((item) => item.orderStatus === status).length
                ),
            },
        ],
    };
        ...
        datasets: [
            {
                ...
                data: [outOfStock, Math.max(0, safeProducts.length - outOfStock)],
            },
        ],
    };
        ...
        datasets: [
            {
                ...
                data: categories.map((cat) =>
                    safeProducts.filter((item) => item.category === cat).length
                ),
            },
        ],
    };
        ...
        <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-6">
                <div className="flex flex-col bg-purple-600 text-white gap-2 rounded-xl shadow-lg hover:shadow-xl p-6">
                    ...
                    <h2 className="text-2xl font-bold">₹{totalAmount ? totalAmount.toLocaleString() : "0"}</h2>
                </div>
